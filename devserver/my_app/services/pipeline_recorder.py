...
class LivePipelineRecorder:
    ...
    async def download_and_save_all_from_comfyui(
        self,
        prompt_id: str,
        media_type: str,
        config: str,
        ...
        """
        Download ALL media outputs from ComfyUI workflow and save as entities.

        This is used for legacy workflows that generate multiple outputs
        (e.g., intermediate steps, multiple variations).

        Unlike download_and_save_from_comfyui() which downloads only the first file,
        this method downloads ALL outputs for complete workflow preservation.
        ...
        Returns:
            List of filenames for all saved entities (empty list if none saved)
        """
        try:
            from my_app.services.comfyui_client import get_comfyui_client
            ...
            logger.info(f"[RECORDER] Waiting for legacy workflow completion: {prompt_id}")
            history = await client.wait_for_completion(prompt_id, timeout=300)

            if not history:
                logger.error(f"[RECORDER] ✗ No history returned for {prompt_id}")
                return []

            # Get ALL generated files (not just first)
            if media_type == 'image':
                files = await client.get_generated_images(history)
            elif media_type in ['audio', 'music']:
                files = await client.get_generated_audio(history)
            ...
            logger.info(f"[RECORDER] Found {len(files)} {media_type} file(s) to download")

            # Download ALL files (not just first)
            saved_filenames = []
            for idx, file_info in enumerate(files):
                try:
                    logger.info(f"[RECORDER] Downloading file {idx+1}/{len(files)}: {file_info.get('filename', 'unknown')}")
                    ...
                    if not file_data:
                        logger.warning(f"[RECORDER] ⚠ Failed to download file {idx+1}, skipping")
                        continue
                    ...
                    # Save as entity (each file gets unique sequence number)
                    filename = self.save_entity(
                        entity_type=f'output_{media_type}',
                        content=file_data,
                        metadata=metadata
                    ...
                    logger.info(f"[RECORDER] ✓ Saved {media_type} {idx+1}/{len(files)} ({len(file_data)} bytes)")

                except Exception as e:
                    logger.error(f"[RECORDER] ✗ Error downloading file {idx+1}: {e}")
                    # Continue with next file (partial success is OK for legacy workflows)
                    continue

            logger.info(f"[RECORDER] Downloaded {len(saved_filenames)}/{len(files)} {media_type} files successfully")
            return saved_filenames

        except Exception as e:
            logger.error(f"[RECORDER] Error in download_and_save_all_from_comfyui: {e}")
            import traceback
            traceback.print_exc()
            return []
