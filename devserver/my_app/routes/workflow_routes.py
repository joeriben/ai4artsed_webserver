...
@workflow_bp.route('/pipeline_configs_metadata', methods=['GET'])
def pipeline_configs_metadata():
    """
    Get metadata for all pipeline configs (Expert Mode UI)
    Reads directly from config files - NO data duplication
    """
    try:
        import sys
        from pathlib import Path
        ...
        if not config_loader.base_path:
        ...
        return jsonify({
            ...
            "count": len(configs_metadata)
        })

    except Exception as e:
        logger.error(f"Error getting pipeline configs metadata: {e}")
        return jsonify({"error": "Failed to get pipeline configs metadata"}), 500


@workflow_bp.route('/pipeline_config/<config_name>', methods=['GET'])
def pipeline_config_detail(config_name):
    """
    Get detailed information for a specific pipeline config
    Reads directly from config file - NO data duplication
    """
    try:
        import sys
        from pathlib import Path
        ...
        schemas_path = Path(__file__).parent.parent.parent / "schemas"
        config_file = schemas_path / "configs" / f"{config_name}.json"

        if not config_file.exists():
            return jsonify({"error": f"Config '{config_name}' not found"}), 404

        # Read config data directly
        with open(config_file, 'r', encoding='utf-8') as f:
            config_data = json.load(f)

        # Return complete config data (frontend can use whatever it needs)
        return jsonify({
            "success": True,
            "config": config_data
        })
        ...
        logger.error(f"Error getting pipeline config detail: {e}")
        return jsonify({"error": "Failed to get pipeline config detail"}), 500
        ...
        if workflow_name and workflow_name.startswith("dev/"):
            ...
            try:
                ...
                if result.status.value == 'completed':
                    ...
                    # ===== AUTO-POST-PROCESSING: Auto-Media-Generation (Output-Chunk System) =====
                    # If no media was generated by pipeline, check config's media_preferences
                    if not prompt_id:
                        logger.info(f"[AUTO-MEDIA] No media generated by pipeline '{schema_name}'")

                        # Get config's media preferences
                        resolved_config = executor.config_loader.get_config(schema_name)
                        default_output = resolved_config.media_preferences.get('default_output', 'text') if resolved_config and resolved_config.media_preferences else 'text'

                        logger.info(f"[AUTO-MEDIA] Config default_output: {default_output}")

                        # Check hidden_commands from original prompt for media type (overrides config)
                        requested_media_type = default_output
                        if hidden_commands.get('image'):
                            requested_media_type = 'image'
                            logger.info(f"[AUTO-MEDIA] User requested image via #image# tag")
                        elif hidden_commands.get('audio'):
                            requested_media_type = 'audio'
                            logger.info(f"[AUTO-MEDIA] User requested audio via #audio# tag")
                        elif hidden_commands.get('music'):
                            requested_media_type = 'music'
                            logger.info(f"[AUTO-MEDIA] User requested music via #music# tag")
                        elif hidden_commands.get('video'):
                            requested_media_type = 'video'
                            logger.info(f"[AUTO-MEDIA] User requested video via #video# tag")

                        # Use Output-Config Selector to find appropriate output config
                        if requested_media_type != 'text':
                            from schemas.engine.output_config_selector import get_output_config_selector

                            selector = get_output_config_selector()
                            output_config_name = selector.select_output_config(requested_media_type, mode)

                            if output_config_name:
                                logger.info(f"[AUTO-MEDIA] Selected output config: {output_config_name} for {requested_media_type} + {mode}")

                                # Execute output pipeline with selected config
                                try:
                                    output_result = asyncio.run(executor.execute_pipeline(
                                ...
                                logger.warning(f"[AUTO-MEDIA] No output config available for {requested_media_type} + {mode}")
                                media_type = "text"  # Fallback to text
                        else:
                            logger.info(f"[AUTO-MEDIA] Media type is 'text', no media generation needed")
                            media_type = "text"
