...
!! Diese Struktur ist noch inkonsistent, oder? Was ist der genauer UNterschied zwischen "manipulate" und "prompt_interception" (und den unter-Prompt-interceptions? Das sollten doch die Configs regeln ob es um Lyrik oder anderes geht)? gibt e seinen Unterschie zwischen "manipulate" und "translate", bzw. ist "translate" nicht einfach ein "manipulate" mit Kontext "Übersetze in Sprache / En ..." und Task-Anweisung? !!
...
!! JETZT ERinnere ich mich wozu "instruction_types" da waren: v.a. zur Festlegung von LLM-Varianten für verschiedene Aufgabentypen (multilingual, reasoning, kulturelle Kompetenzen), auch über verschiedenen API/lokale Server hinweg. Denn da haben LLM unterschiedlcihen Pfade/Bezeichnungen etc.! "Instruction_types" hat das als Name nicht kommuniziert, und es war offenbar auch nicht kommuniziert. Jeder Chunk - aber nicht die Configs! - sollte in seinen Metadaten seine primäre funktionale Verwendung nennen (z.B. "translation", "complex transformation/evaluation", "evaluation", "image analysis" -> FINDE DIE TYPEN IM BACKUP-Ordner /home/joerissen/ai/ai4artsed_webserver (Copy 4)/devserver !). Und dann würde in der Tat eine zentrale Datei eine zu bevorzugende Auswahl an LLM festlegen -> die müsste das aber auch explizit in ihrem Namen zum Ausdruck bringen -> oder aber das kommt in eine zentrale Config-Datei für den gesamten Server (könnte aber leicht unübersichtlich werden). Wir hätten also z.B. eine LLM_config Datei im root oder in einem /config Ordner. "instruction_types" sollte nicht wieder eingeführt werden, aber eine meta-variable "llm application type" oder ähnlich, in den metadaten der chunks. Die Aufgabe wäre also hier,  jedem Chunk immer das beste LLM für seinen Zweck anzubieten, ohne dass das LLM dort starr festgeschrieben wird. So können wir z.B. alle 3 Monate dem Technikfortschritt folgen und LLM dort definieren für die von uns implementierten lokalen LLM Server und externen API-ANbieter. -> Todo: dieses System stabil implementieren: 1) Finde heraus wo Meta-INformation für llm-Typendefinition hingehören - Chunks korrekt? Oder müssen sie auch in die Configs, was ich nicht hoffe? 2) Finde geeignete Namen und implementiere dieses Meta-Datum in den Chunk-Dateien. 3) Baue eine llm_config auf und liste dort die passenden LLM (fehlt Dir dies Info, schaue in der alten Version nach unter /home/joerissen/ai/ai4artsed_webserver (Copy 4)/devserver ). 4) IMplementiere eine Funktion zum Abruf der konkreten LLM-Namen zur Laufzeit während der Chunk-Verarbeitung, diskriminiert nach aktuell verwendeter Lösung: lokal vs. api. Das ist etwas komplexer. Ggf. hatte die alte instruction_type-routine hierfür bereits alles korrekt implementiert, bevor wir es gelöscht hatten. Implementiere diese Diskriminierung an der Stelle die den konkreten serverport/API-Aufruf durchführt. Sie entscheidet je nach voreingestellten Service, welcher Modellname zu verwenden ist. !!
...
!! Das ist mit unklar, wieso all diese aliases? Wieso ist "Task" und "COntext" hier dasselbe?? !!
...
!! Siehe oben, redundante Pipelines hier? "Manipulation" vs. "interceptioN" gut begründet? "interception_single" vs. "simple_interception"? Hier hat eder vorangehende Task nicht sehr konsistent, oder nicht gut erkennbar konsistent gearbeitet. Geade hier brauchen wir aber auch maximale logische Klarheit !!
...
!! obligatorische Übersetzung -> via pipeline hier, nur nicht wenn #notranslate# -> also hidden prompt-Abfrage hier VOR der Übersetzung. Dann Übersetzung bzw. ggf. auch komplexre Textmanipulation (Safety-Check kann hier integriert werden. Dann erst Config loader. !!
                           ↓
┌──────────────────────────────────────────────────────────────┐
│                   Config Loader                              │
