  ...
  de: {
    ...
    latentLab: {
      ...
      textLab: {
        ...
        explainHowTitle: 'Wie benutze ich das Tool?',
        explainHowText: 'Beim Öffnen der Seite wird automatisch ein Standardmodell (Llama 3.2 3B) geladen. Über „Modell wechseln" in der Statusleiste kannst du jederzeit ein anderes Preset oder eine eigene HuggingFace-ID laden. Drei Tabs stehen zur Verfügung: Tab 1 (RepEng) definiert Kontrastpaare und findet Konzept-Richtungen. Tab 2 (Vergleich) lädt zwei Modelle und vergleicht ihre inneren Repräsentationen. Tab 3 (Bias) führt systematische Bias-Experimente durch.',
        modelPanel: {
          ...
          autoLoading: 'Standardmodell wird geladen (Llama 3.2 3B)...',
          autoLoadFailed: 'Automatisches Laden fehlgeschlagen',
        },
  ...
  en: {
    ...
    latentLab: {
      ...
      textLab: {
        ...
        explainHowTitle: 'How do I use this tool?',
        explainHowText: 'A default model (Llama 3.2 3B) is loaded automatically when the page opens. You can switch to a different preset or custom HuggingFace ID via "Switch model" in the status bar at any time. Three tabs are available: Tab 1 (RepEng) defines contrast pairs and finds concept directions. Tab 2 (Compare) loads two models and compares their internal representations. Tab 3 (Bias) runs systematic bias experiments.',
        modelPanel: {
          ...
          autoLoading: 'Loading default model (Llama 3.2 3B)...',
  ...
  tr: {
    ...
    latentLab: {
      ...
      textLab: {
        ...
        explainHowTitle: 'Bu aracı nasıl kullanırım?',
        explainHowText: 'Sayfa açıldığında otomatik olarak bir varsayılan model (Llama 3.2 3B) yüklenir. Durum çubuğundaki "Model değiştir" ile istediğiniz zaman farklı bir hazır ayar veya özel HuggingFace kimliği yükleyebilirsiniz. Üç sekme mevcuttur: Sekme 1 (RepEng) kontrast çiftleri tanımlar ve kavram yönleri bulur. Sekme 2 (Karşılaştır) iki model yükler ve iç temsillerini karşılaştırır. Sekme 3 (Önyargı) sistematik önyargı deneyleri yürütür.',
        modelPanel: {
          ...
          autoLoading: 'Varsayılan model yükleniyor (Llama 3.2 3B)...',
          autoLoadFailed: 'Otomatik yükleme başarısız',
        },
  ...
  ko: {
    ...
    latentLab: {
      ...
      textLab: {
        ...
        explainHowTitle: '이 도구를 어떻게 사용하나요?',
        explainHowText: '페이지를 열면 기본 모델(Llama 3.2 3B)이 자동으로 로드됩니다. 상태 표시줄의 "모델 전환"을 통해 언제든지 다른 프리셋이나 사용자 정의 HuggingFace ID로 전환할 수 있습니다. 세 개의 탭을 사용할 수 있습니다: 탭 1(RepEng)은 대조 쌍을 정의하고 개념 방향을 찾습니다. 탭 2(비교)는 두 모델을 로드하고 내부 표현을 비교합니다. 탭 3(바이어스)는 체계적인 바이어스 실험을 수행합니다.',
        modelPanel: {
          title: '모델 관리',
          presetLabel: '프리셋',
          ...
          switchModel: '모델 전환',
          autoLoading: '기본 모델 로드 중 (Llama 3.2 3B)...',
          autoLoadFailed: '자동 로드 실패',
        },
  ...
  uk: {
    ...
    latentLab: {
      ...
      textLab: {
        ...
        explainHowTitle: 'Як користуватися цим інструментом?',
        explainHowText: 'При відкритті сторінки автоматично завантажується модель за замовчуванням (Llama 3.2 3B). Через "Змінити модель" у рядку стану можна будь-коли завантажити інший набір або власний HuggingFace ID. Доступні три вкладки: Вкладка 1 (RepEng) визначає контрастні пари та знаходить напрямки концептів. Вкладка 2 (Порівняння) завантажує дві моделі та порівнює їх внутрішні представлення. Вкладка 3 (Упередженість) запускає систематичні експерименти з упередженістю.',
        modelPanel: {
          ...
          autoLoading: 'Завантаження моделі за замовчуванням (Llama 3.2 3B)...',
          autoLoadFailed: 'Автоматичне завантаження не вдалося',
        },
  ...
  fr: {
    ...
    latentLab: {
      ...
      textLab: {
        ...
        explainHowTitle: 'Comment utiliser cet outil ?',
        explainHowText: 'Un modèle par défaut (Llama 3.2 3B) est chargé automatiquement à l\'ouverture de la page. Vous pouvez passer à un autre préréglage ou un ID HuggingFace personnalisé via « Changer de modèle » dans la barre d\'état. Trois onglets sont disponibles : Onglet 1 (RepEng) définit des paires de contraste et trouve des directions de concepts. Onglet 2 (Comparer) charge deux modèles et compare leurs représentations internes. Onglet 3 (Biais) exécute des expériences systématiques de biais.',
        modelPanel: {
          ...
          autoLoading: 'Chargement du modèle par défaut (Llama 3.2 3B)...',
          autoLoadFailed: 'Le chargement automatique a échoué',
        },
