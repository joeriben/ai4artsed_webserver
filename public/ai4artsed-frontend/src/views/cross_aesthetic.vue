<template>
  <div class="cross-aesthetic-page">
    <h1 class="page-title">{{ t('crossAesthetic.title') }}</h1>
    <p class="page-subtitle">{{ t('crossAesthetic.subtitle') }}</p>
    ...
    <div class="strategy-tabs">
      ...
      >
        ...
        <span class="strategy-desc">{{ t(`crossAesthetic.strategies.${s.id}.short`) }}</span>
      </button>
    </div>

    <!-- Strategy A: Image → Audio -->
    <div v-if="activeStrategy === 'a'" class="strategy-panel">
      <h2>{{ t('crossAesthetic.strategies.a.title') }}</h2>
      <p class="strategy-description">{{ t('crossAesthetic.strategies.a.description') }}</p>

      <div class="input-group">
        <label>{{ t('crossAesthetic.imageUpload') }}</label>
      ...
      <button class="generate-btn" :disabled="!imageBase64 || generating" @click="runStrategyA">
        {{ generating ? t('crossAesthetic.generating') : t('crossAesthetic.generate') }}
      </button>
    </div>

    <!-- Strategy B: Shared Seed -->
    <div v-if="activeStrategy === 'b'" class="strategy-panel">
      <h2>{{ t('crossAesthetic.strategies.b.title') }}</h2>
      <p class="strategy-description">{{ t('crossAesthetic.strategies.b.description') }}</p>

      <div class="input-group">
        <label>{{ t('crossAesthetic.prompt') }}</label>
    ...
    <div v-if="activeStrategy === 'c'" class="strategy-panel">
      <h2>{{ t('crossAesthetic.strategies.c.title') }}</h2>
      <p class="strategy-description">{{ t('crossAesthetic.strategies.c.description') }}</p>
      ...
      <div class="params-row">
        <div class="param">
          <label>{{ t('crossAesthetic.direction') }}</label>
          <select v-model="params.direction">
            <option value="image_to_audio">{{ t('crossAesthetic.imageToAudio') }}</option>
            <option value="audio_to_image">{{ t('crossAesthetic.audioToImage') }}</option>
          </select>
        </div>
        <div class="param">
          <label>Seed</label>
...
const GPU_BASE = import.meta.env.DEV ? 'http://localhost:17803' : '/gpu'
...
async function gpuPost(path: string, body: Record<string, unknown>) {
  const resp = await fetch(`${GPU_BASE}${path}`, {
    method: 'POST',
  ...
  if (!resp.ok) {
    ...
    throw new Error(`GPU service error ${resp.status}: ${text}`)
  }
  return resp.json()
}
...
async function runStrategyA() {
  ...
  try {
    const result = await gpuPost('/api/cross_aesthetic/image_to_audio', {
...
async function runStrategyB() {
  ...
  generating.value = true
  try {
    const result = await gpuPost('/api/cross_aesthetic/shared_seed', {
...
async function runStrategyC() {
  ...
  generating.value = true
  try {
    const result = await gpuPost('/api/cross_aesthetic/cross_decode', {
      prompt: params.prompt,
