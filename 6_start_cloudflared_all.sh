#!/bin/bash
# Start Cloudflared tunnel with ALL services: lab + werkraum + comfyui

TUNNEL_NAME="werkraum-tunnel"
TEMP_CONFIG="/tmp/cloudflared-all-services.yml"
CREDENTIALS_FILE="$HOME/.cloudflared/b614ccb7-c8f3-4831-bfbb-d4674a0e2749.json"

echo "=== Cloudflared Tunnel Startup Check (All Services) ==="
echo ""
...
# Create temporary config with all three services
...
ingress:
  ...
  # ComfyUI API (port 7801)
  - hostname: comfyui.ai4artsed.org
    service: http://127.0.0.1:7801
    originRequest:
      httpHostHeader: comfyui.ai4artsed.org
      connectTimeout: 60s
      tcpKeepAlive: 30s
      noHappyEyeballs: false
      keepAliveConnections: 100
...
cloudflared tunnel ingress rule https://comfyui.ai4artsed.org --config ${TEMP_CONFIG}
echo ""

echo "Starting Cloudflared tunnel with ALL services:"
echo "  - lab.ai4artsed.org → http://127.0.0.1:17801"
echo "  - werkraum.ai4artsed.org → http://127.0.0.1:5000"
echo "  - comfyui.ai4artsed.org → http://127.0.0.1:7801"
echo ""
